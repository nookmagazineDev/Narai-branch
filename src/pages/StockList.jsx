...
const UP100_ALL_COVERS_CODES = new Set([
  ...
  '11000145', // เสิร์ฟให้ลูกค้าทุกราคา ยอดเบิกจึงต้องคูณหัวรวม ไม่ใช่หัวราคาสูงล้วน
  '1000198',  // FC เนื้อมะพร้าวหมักนุ่มข้าน้อย 1กก/แพ็ค — เสิร์ฟทุกราคาเหมือนกัน
]);
...
export default function StockList() {
  ...
  const handleBranchChange = (branch) => {
    // สลับสาขาแล้วจำนวนหัวที่กรอกค้างไว้จะถูกของจากเซิร์ฟเวอร์ของสาขาใหม่ทับทิ้งเงียบๆ
    // (loadSpecialPcts -> specialPcts -> เซ็ต pctInputMap ใหม่ทั้งชุด)
    //
    // เคสจริงที่ทำให้เสียงาน: ส่วนกลางไล่ตั้งค่าหัวให้หลายสาขาติดกัน กรอกสาขาแรกเสร็จแล้ว
    // สลับไปสาขาถัดไปโดยยังไม่กดบันทึก ค่าที่กรอกของสาขาก่อนหน้าหายไปทั้งชุด
    // พอกดบันทึกที่สาขาสุดท้ายจึงติดแค่สาขานั้น กลายเป็น "บันทึกของแต่ละสาขาไม่ติด"
    if (branch !== selectedBranch && canEditCovers && pendingCoverDays > 0) {
      const go = window.confirm(
        `สาขา ${selectedBranch} มี ${pendingCoverDays} วันที่แก้จำนวนหัวลูกค้าไว้แต่ยังไม่ได้บันทึก\n` +
        'ถ้าสลับสาขาตอนนี้ ค่าที่กรอกไว้จะหายทั้งหมด\n\n' +
        'กด "ยกเลิก" เพื่อกลับไปกดปุ่มบันทึกก่อน · กด "ตกลง" เพื่อสลับสาขาและทิ้งค่าที่กรอกไว้'
      );
      if (!go) return;
    }
