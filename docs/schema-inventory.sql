/* ============================================================================
   ...
   3) ทุกตารางที่อ้างถึงสินค้า เก็บรหัสไว้ "สองคอลัมน์" คนละหน้าที่กัน
        item_code       รหัสจริงที่เอาไว้แสดงและทำรายงาน — มี 0 นำหน้าครบ
        item_code_norm  รหัสที่ตัด 0 นำหน้าแล้ว มีไว้ join อย่างเดียว ห้ามเอาไปแสดง
      ที่ต้องมีสองอันเพราะรหัสในชีทปนกันทั้งแบบตัวเลข (5001) และข้อความ ('05001')
      Apps Script เองก็ตัด 0 ก่อนจับคู่ทุกครั้ง (ฟังก์ชัน normalizeId)
      ถ้าไม่เก็บค่าที่ normalize ไว้ การ join จะพลาดเงียบๆ แบบเดียวกับที่เคยเจอ
      ตอนย้ายตารางงานครั้งแรก (กะ 72% หาเจ้าของไม่เจอ)

      แถวไหนที่ชีทเก็บรหัสเป็นตัวเลขจน 0 นำหน้าหายไปตั้งแต่ต้นทาง สคริปต์ย้ายข้อมูล
      จะเติมกลับให้ตอนจบ โดยลอกรหัสตัวเต็มจาก inv_item (แท็บ item เก็บเป็นข้อความ
      จึงมี 0 ครบ) มาใส่ทุกตารางที่จับคู่ด้วย item_code_norm ได้
