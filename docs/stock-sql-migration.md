...
| เฟส | สิ่งที่ทำ | สถานะ |
|---|---|---|
| 1 | ตาราง + สคริปต์ย้ายข้อมูลเก่า | **มีแล้ว** — ยังไม่ได้รันบนเครื่องจริง |
| 2 | อ่านจาก SQL (`getStockItems`, `getStockTotal`) | **มีแล้วฝั่งเซิร์ฟเวอร์** — หน้าเว็บยังไม่ได้เรียกใช้ |
| 3 | บันทึกลง SQL แล้ว mirror กลับชีท | ยังไม่ได้ทำ |
| 4 | ตัดชีทออก (dashboard/ปิดรอบสิ้นเดือน อ่าน SQL) | ยังไม่ได้ทำ |

**ยังห้ามเปิดให้หน้าเว็บอ่านจาก SQL จนกว่าจะทำเฟส 3** — เพราะการกดบันทึกยังเขียนลงชีทอย่างเดียว
ถ้าเปิดให้อ่าน SQL ตอนนี้ สาขานับสต๊อกเสร็จแล้วกดบันทึก หน้าจะยังโชว์ยอดเก่าที่ย้ายมาครั้งล่าสุด
เหมือนบันทึกไม่ติด ซึ่งแย่กว่าช้า
...
| `saveStock` | บันทึกการนับ + ใบเบิก + อัปเดตยอดยกมา | 3 |
| `updateStorageCategory` | ตั้งหมวดจัดเก็บของสาขา | 3 |
| `saveAvgPerHead` | ค่าเฉลี่ยยอดใช้ต่อหัว (สูตรเบิก) | 3 |
| `saveBranchPercentagesBulk` | เปอร์เซ็นต์การเบิกของแต่ละสาขา | 3 |
