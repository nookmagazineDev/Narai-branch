...
// ตั้ง env USAGE_API_BASE บน Vercel เพื่อทับค่าเริ่มต้นได้ (เช่นเวลาย้ายเครื่อง/เปลี่ยนทางเข้า)
//
// ค่าเริ่มต้นคือ Cloudflare Tunnel ที่ชี้เข้าเครื่อง NARAI-PIZZARIA ซึ่งเป็นเครื่องที่รัน
// office-server รุ่นปัจจุบันและมี SQL Server อยู่ในตัว — ย้ายมาจาก http://storenarai.dyndns.tv:8787
// เพราะโฮสต์เดิมตอบด้วย office-server รุ่นเก่าที่ไม่มี action ของยูนิฟอร์ม (ตอบ "ไม่รู้จักคำสั่ง")
// ทั้งที่หน้าเว็บรุ่นใหม่ deploy ไปแล้ว
//
// ที่ต้องแก้ค่าเริ่มต้นไม่ใช่แค่ตั้ง env: env ที่บันทึกไว้เฉพาะ Preview จะไม่มีผลกับ Production
// ตัว Production จึงถอยมาใช้ค่าเริ่มต้นในไฟล์นี้เงียบ ๆ ต่อไป — ค่าเริ่มต้นจึงต้องเป็นตัวที่ถูกต้อง
// ไม่ใช่ตัวที่เลิกใช้แล้ว (ตั้ง env ทับยังทำได้เหมือนเดิมเมื่อต้องย้ายอีก)
export const USAGE_API_BASE = process.env.USAGE_API_BASE || 'https://usage.khanoykorshabu.com';
