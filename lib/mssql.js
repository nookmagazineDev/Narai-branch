...
function buildConfig() {
  ...
  //
  // วิธีนี้ใช้ไม่ได้เมื่อต่อจากนอกออฟฟิศ เพราะต้องเปิด UDP 1434 ออกเน็ตด้วย
  // และ SQL Express ตั้งค่าเริ่มต้นเป็น "TCP Dynamic Ports" คือเปลี่ยนพอร์ตทุกครั้งที่รีสตาร์ต
  // จึง forward พอร์ตที่เราเตอร์ไม่ได้ -> อาการคือต่อไม่ติดแบบหมดเวลา (ETIMEOUT/ConnectionError)
  //
  // จึงให้ HR_DB_PORT ที่ตั้งไว้ชนะ: ถ้าระบุพอร์ตมาตรงๆ ให้ต่อพอร์ตนั้นเลย ไม่ต้องพึ่ง SQL Browser
  // (ต้องไปตั้ง TCP Port = 1433 แบบคงที่ใน SQL Server Configuration Manager ให้ตรงกัน)
  if (process.env.HR_DB_INSTANCE && !process.env.HR_DB_PORT) {
