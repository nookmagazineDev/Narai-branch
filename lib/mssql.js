// pool MS SQL Server สำหรับข้อมูล HR (ตารางงาน/พนักงาน) — เครื่อง inventory.dyndns.tv
//
// ใช้ชื่อโฮสต์ ไม่ใช่ IP: เครื่องนี้เป็น dynamic DNS (ตอนนี้ชี้ไป 203.154.185.48)
// ถ้า hardcode IP ไว้ วันไหน IP เปลี่ยนจะพังเงียบๆ เป็น 'ต่อไม่ติดในเวลาที่กำหนด'
// lib/mysql.js ต่อเครื่องเดียวกันด้วยชื่อนี้อยู่แล้ว จึงใช้ให้ตรงกัน
...
//   HR_DB_HOST (ค่าเริ่มต้น inventory.dyndns.tv), HR_DB_PORT (1433), HR_DB_NAME (narai_hr),
//   HR_DB_USER, HR_DB_PASSWORD, HR_DB_ENCRYPT=1 (ถ้าเปิด TLS ที่ SQL Server แล้ว)
//
// อย่าตั้ง HR_DB_INSTANCE บน Vercel — มันจะทำให้ต่อผ่าน SQL Browser (UDP 1434) ซึ่งจากนอกออฟฟิศไม่ติด
// ตอนนี้ต้องตั้ง HR_DB_USE_BROWSER=1 คู่กันด้วยถึงจะมีผล ค่าเริ่มต้นคือต่อพอร์ตตรงเสมอ

import sql from 'mssql';

let poolPromise = null;

/* สคริปต์ที่ต้องคุยกับฐานข้อมูลอื่นบนเครื่องเดียวกัน (เช่น InventoryNarai ของหน้านับสต๊อก)
   สั่งเปลี่ยนฐานข้อมูลได้ด้วย useDatabase() ก่อนต่อครั้งแรก — host/login ใช้ชุดเดิมทั้งหมด */
let databaseOverride = null;

/**
 * เปลี่ยนฐานข้อมูลปลายทางของ pool นี้ ต้องเรียกก่อน getPool() ครั้งแรก
 * (เรียกทีหลังจะไม่มีผลกับ pool ที่ต่อไปแล้ว ซึ่งเป็นความพังแบบเงียบๆ จึงโยน error ทิ้งไปเลย)
 */
export function useDatabase(name) {
  if (poolPromise) throw new Error('ต้องเรียก useDatabase() ก่อนต่อฐานข้อมูลครั้งแรก');
  databaseOverride = name || null;
}

function buildConfig() {
  const cfg = {
    server: process.env.HR_DB_HOST || 'inventory.dyndns.tv',
    port: Number(process.env.HR_DB_PORT) || 1433,
    user: process.env.HR_DB_USER,
    password: process.env.HR_DB_PASSWORD,
    database: databaseOverride || process.env.HR_DB_NAME || 'narai_hr',
  ...
  // named instance (เช่น SQLEXPRESS) จะไม่ต่อพอร์ตตรง แต่ไปถาม SQL Browser ที่ UDP 1434 ก่อน
  // ว่าอินสแตนซ์นั้นใช้พอร์ตอะไร แล้วค่อยต่อพอร์ตที่ได้มา
  //
  // วิธีนี้ใช้จากนอกออฟฟิศไม่ได้ ต้องเปิด UDP 1434 ออกเน็ตด้วย ซึ่งปกติไม่มีใคร forward
  // อาการที่ได้คือต่อไม่ติดแบบหมดเวลา (ConnectionError/ETIMEOUT) ทั้งที่ SQL ปกติดี
  // และ SQL Browser ก็รันอยู่ที่เครื่องปลายทางจริงๆ จึงหลงคิดว่าตั้งถูกแล้ว
  //
  // เครื่อง NARAI-PIZZARIA\SQLEXPRESS ตั้ง TCP Port = 1433 คงที่อยู่แล้ว (ยืนยันจาก
  // sys.dm_exec_connections.local_tcp_port = 1433 และ netstat 0.0.0.0:1433 LISTENING)
  // จึงไม่ต้องพึ่ง SQL Browser เลย ต่อพอร์ตตรงได้
  //
  // ค่าเริ่มต้นจึง "ต่อพอร์ตตรงเสมอ" ถ้าจำเป็นต้องใช้ SQL Browser จริงๆ ค่อยตั้ง
  // HR_DB_USE_BROWSER=1 เพิ่ม — ไม่ให้ HR_DB_INSTANCE ที่ค้างอยู่ทำให้พังเงียบๆ อีก
  if (process.env.HR_DB_INSTANCE && String(process.env.HR_DB_USE_BROWSER || '') === '1') {
 ...
 *
 * มีไว้เพราะ "ต่อไม่ติด" อาจเกิดจาก env พิมพ์ผิด (เว้นวรรคท้าย IP, พอร์ตผิด)
 * ซึ่งดูจากข้อความเฉยๆ แยกไม่ออกเลยว่ายิงไปที่ไหนกันแน่
 */
export function describeTarget() {
  const cfg = buildConfig();
  if (cfg.options?.instanceName) return `${cfg.server}\\${cfg.options.instanceName}/${cfg.database} (ผ่าน SQL Browser)`;
  return `${cfg.server}:${cfg.port}/${cfg.database}`;
}
